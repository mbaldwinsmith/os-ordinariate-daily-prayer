...
## Office of Readings — Year I / Ordinary Time only (scope explicitly limited)

- **Status: UNVERIFIED, best-effort reconstruction, and deliberately partial.** User-approved
  scope for this pass: Year I / Ordinary Time only (34 weeks). Year II and the Advent/
  Christmas/Lent/Triduum/Easter seasons are not built yet - `src/officeOfReadings.ts`
  returns `null` for anything outside this scope rather than guessing, and the UI says so
  plainly rather than silently showing nothing.
- **No authoritative lectionary index reachable**: same network-sandbox limitation as the
  Psalter and psalter skeleton above - no GitHub-hosted Office of Readings lectionary index
  turned up either.
- **Generated by**: `scripts/generate-office-of-readings.mjs` (`npm run generate:office-of-readings`)
  → `data/office-of-readings/yearI/ordinaryTime/week{1..34}/{sunday,...,saturday}.json`
  (238 files). Every file is `"verified": false`.
- **Approach**: one real Old Testament book per week (`WEEK_BOOKS` in the generation
  script - Genesis through Zechariah, a real, sensible cross-section of the canon), stepping
  through consecutive chapters day by day (wrapping if a book has fewer than 7 chapters,
  e.g. Ruth, Joel). This mirrors the general shape of how the actual Office of Readings
  First Reading works (continuous OT reading through Ordinary Time) but is **not** a
  transcription of the official week-by-week assignment - which specific book falls on
  which specific week is unverified, same category of gap as the psalter skeleton's
  day-to-day pairing.
- **Patristic/hagiographic second readings: omitted entirely** (`patristicReading: null`
  throughout) - the pragmatic MVP fallback TASKS.md itself suggests, since the DRC text has
  no patristic content and sourcing one is a separate decision not made yet.
- **`data/texts/book-abbreviations.json`** (new): extracted the book-abbreviation table from
  `CONVENTIONS.md` into a shared, schema-validated data file so the generation script and
  ...
  generated `scriptureReading.ref` actually resolves - not just that the JSON is
  schema-shaped.
